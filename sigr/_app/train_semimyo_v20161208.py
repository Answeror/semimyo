...
#  def _infer_shape(self, *args, **kargs):
    #  names = self.get_internals().list_outputs()
    #  kargs = {k: v for k, v in kargs.items() if k in names}
    #  return infer_shape_orig(self, *args, **kargs)


#  def _infer_type(self, *args, **kargs):
    #  names = self.get_internals().list_outputs()
    #  kargs = {k: v for k, v in kargs.items() if k in names}
    #  return infer_type_orig(self, *args, **kargs)


#  infer_shape_orig = mx.symbol.Symbol.infer_shape
#  mx.symbol.Symbol.infer_shape = _infer_shape
#  infer_type_orig = mx.symbol.Symbol.infer_type
#  mx.symbol.Symbol.infer_type = _infer_type


#  def _get_executor_group(symbol, contexts, workload, data_shapes, label_shapes,
                        #  param_names, for_training, inputs_need_grad,
                        #  shared_group=None, input_types=None,
                        #  *args, **kargs):
    #  names = symbol.list_arguments()
    #  data_shapes = [(k, v) for k, v in data_shapes if k in names]
    #  label_shapes = [(k, v) for k, v in label_shapes if k in names]
    #  if input_types:
        #  input_types = {k: v for k, v in input_types.items() if k in names}
    #  return get_executor_group_orig(symbol, contexts, workload, data_shapes, label_shapes,
                                   #  param_names, for_training, inputs_need_grad,
                                   #  shared_group, input_types,
                                   #  *args, **kargs)


#  get_executor_group_orig = mx.module.module.DataParallelExecutorGroup
#  mx.module.module.DataParallelExecutorGroup = _get_executor_group
...
@click.option('--batch-norm-momentum', type=float,
...
@click.option('--static-net')
@click.option('--dynamic-net')
@click.option('--spot-net')
@click.option('--glove-branch-at',
              type=click.Choice(constant.GLOVE_BRANCH_AT_CHOICES),
              default=constant.GLOVE_BRANCH_AT)
...
@click.option('--static-loss-weight', type=float)
@click.option('--dynamic-loss-weight', type=float)
@click.option('--glove-loss-weight', type=float)
@click.option('--spot-loss-weight', type=float)
@click.option('--num-glove-layer', type=int)
@click.option('--num-glove-hidden', type=int)
@click.option('--num-gesture-layer', type=int)
@click.option('--num-gesture-hidden', type=int)
    ...
    with Context(args.log, parallel=True):
        ...
        mod = get_module(
            ...
            symbol_kargs=dict(
                ...
                batch_norm_use_global_stats=args.batch_norm_use_global_stats,
                shared_net=args.shared_net,
                gesture_net=args.gesture_net,
                glove_net=args.glove_net,
                static_net=args.static_net,
                dynamic_net=args.dynamic_net,
                spot_net=args.spot_net,
                glove_branch_at=args.glove_branch_at,
                ...
                num_gesture_layer=args.num_gesture_layer,
                num_gesture_hidden=args.num_gesture_hidden,
                num_mini_batch=args.num_mini_batch,
                gesture_loss_weight=args.gesture_loss_weight,
                static_loss_weight=args.static_loss_weight,
