...
class Dataset(Base):
    ...
    name = 'ninapro-db1-raw/semg-glove-diff01'
    ...
    def get_dataiter(self, combos, **kargs):
        ...
        for combo in combos:
            ...
            glove_diff1.append(trial.data[2])
        ...
        assert semg and glove and glove_diff1, 'Empty data'
        ...
        logger.debug('First segment shape: {}', (semg[0].shape, glove[0].shape, glove_diff1[0].shape))

        semg = np.vstack(semg).reshape(-1, 1, self.num_semg_row, self.num_semg_col)
        glove = np.vstack(glove)
        glove_diff1 = np.vstack(glove_diff1)
...
class GetTrial(object):
    ...
    def __call__(self, combo):
        ...
        semg, glove, glove_diff1, local_gesture = self.memo[path][
        ...
        return Trial(data=(semg, glove, glove_diff1), gesture=gesture, subject=subject)

    def _segment(self, semg, glove, glove_diff1, gesture):
        if self.norest:
        ...
        for begin, end in zip(breaks[:-1], breaks[1:]):
            ...
            if g > 0:
                ...
                data[(g, trials[g])] = (semg[begin:end],
                                        glove[begin:end],
                                        glove_diff1[begin:end],
                                        gesture[begin:end])
        return data
...
def _get_data_aux(path, preprocess):
    ...
    glove_diff1 = np.diff(np.vstack(glove[:1], glove), axis=0)
    gesture = sio.loadmat(path)['restimulus'].astype(np.uint8).ravel()
    if preprocess:
        (semg, glove, glove_diff1, gesture) = preprocess(
            (semg, glove, glove_diff1, gesture),
            **Dataset.get_preprocess_kargs()
        )
    return semg, glove, glove_diff1, gesture
