...
from .get_data import get_ninapro_db1_semg_latent_data
from .common import make_lru_dict
from .. import Combo, Trial, get_index
from ..amplitude_weighting import get_amplitude_weight
from ... import constant, CACHE
...
class Dataset(Base):
    ...
    name = 'ninapro-db1-raw/semg-latent'
    ...
    def get_trial_func(self, **kargs):
        return GetTrial(self.root, self.gestures, self.trials, self.num_latent, **kargs)
    ...
    def get_dataiter(self, combos, **kargs):
        ...
        assert semg and latent, 'Empty data'
        ...
        return DataIter(
            ...
            label=OrderedDict([('gesture', gesture), ('latent', latent)]),
...
class GetTrial(object):
    ...
    def __init__(self, root, gestures, trials, num_latent, preprocess=None, norest=False):
        self.root = root
        self.preprocess = preprocess
        self.memo = make_lru_dict(3)
    ...
    def __call__(self, combo):
        ...
        if path not in self.memo:
            ...
            self.memo.update({path: self._segment(*data) for path, data in
                              zip(paths, get_ninapro_db1_semg_latent_data(
                                  paths, self.preprocess, self.num_latent))})
        data = self.memo[path]
        semg, latent, local_gesture = data[
        ...
        return Trial(data=(semg, latent), gesture=gesture, subject=subject)

    def _segment(self, semg, latent, gesture):
        if self.norest:
        ...
        for begin, end in zip(breaks[:-1], breaks[1:]):
            ...
            if g > 0:
                ...
                data[(g, trials[g])] = (semg[begin:end], latent[begin:end], gesture[begin:end])
