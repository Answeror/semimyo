...
class Dataset(Base):
    ...
    name = 'ninapro-db1-raw/semg-pose'
    ...
    def get_trial_func(self, **kargs):
        return GetTrial(self.root, self.gestures, self.trials, self.num_pose, **kargs)
...
class GetTrial(object):
    ...
    def __init__(self, root, gestures, trials, num_pose, preprocess=None, norest=False):
        self.root = root
        ...
        if path not in self.memo:
            ...
            self.memo.update({path: self._segment(*data) for path, data in
                              zip(paths, get_ninapro_db1_semg_pose_data(
                                  paths, self.preprocess, self.num_pose))})
        data = self.memo[path]
