...
class Dataset(Base):
    ...
    name = 'ninapro-db1-raw/semg-libhand'
...
def _get_data_aux(path, preprocess):
    ...
    glove = sio.loadmat(_get_libhand_path(path))['angle'].astype(np.float32)
    glove = glove[:, :Dataset.num_glove]
...
def _get_libhand_path(path):
    parts = path.split(os.sep)
    return os.sep.join(parts[:-2] + ['libhand'] + parts[-2:])
