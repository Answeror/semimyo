...
class Module(AdaBNMixin, IgnoreInputMixin, Base):
    ...
    def __init__(self, **kargs):
        ...
        self.ignore_label = []
        if symbol_kargs.get('gesture_loss_weight') == 0:
            self.ignore_label.append('gesture')
        if not symbol_kargs.get('glove_loss_weight'):
            self.ignore_label.append('glove')
        if not symbol_kargs.get('spot_loss_weight'):
            self.ignore_label.append('spot')

        super(Module, self).__init__(
            data_names=['semg'],
            label_names=['gesture', 'glove', 'spot', 'gesture_label'],
    ...
    @property
    def has_glove_branch(self):
        return 'glove' not in self.ignore_label

    @property
    def has_spot_branch(self):
        return 'spot' not in self.ignore_label
    ...
    def fit(self, **kargs):
        ...
        if eval_metric is None:
            if self.has_gesture_branch and self.has_glove_branch and self.has_spot_branch:
                eval_metric = [Accuracy(0, 'gesture'), RMSE(1, 'glove'), Accuracy(2, 'spot')]
            elif self.has_gesture_branch and self.has_glove_branch:
                eval_metric = [Accuracy(0, 0), RMSE(1, 1)]
            elif self.has_gesture_branch and self.has_spot_branch:
                eval_metric = [Accuracy(0, 'gesture'), Accuracy(1, 'spot')]
            elif self.has_gesture_branch:
                eval_metric = 'acc'
            elif self.has_glove_branch:
                eval_metric = 'rmse'
            elif self.has_spot_branch:
                eval_metric = 'acc'
