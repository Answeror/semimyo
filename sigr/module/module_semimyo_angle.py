...
class Module(IgnoreInputMixin, AdaBNMixin, BaseModule):
    ...
    def __init__(self, **kargs):
        ...
        if not symbol_kargs.get('angle_loss_weight'):
            self.ignore_label.append('angle')

        super(Module, self).__init__(
            data_names=['semg'],
            label_names=['gesture', 'angle'],
    ...
    @property
    def has_angle_branch(self):
        return 'angle' not in self.ignore_label

    def get_eval_metric(self):
        eval_metric = []
        if self.has_gesture_branch:
            eval_metric.append(Accuracy(len(eval_metric), 'gesture'))
        if self.has_angle_branch:
            eval_metric.append(Loss(len(eval_metric), 'angle'))
        return eval_metric
