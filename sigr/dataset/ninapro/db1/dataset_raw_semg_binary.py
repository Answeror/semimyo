...
from .get_data import get_ninapro_db1_semg_binary_data
...
class Dataset(BaseDataset):
    ...
    def get_dataiter(self, combos, **kargs):
        ...
        assert semg and binary, 'Empty data'
        ...
        nt.assert_equal(len(semg), len(binary))
        nt.assert_equal(binary.ndim, 2)
    ...
    def __call__(self, combo):
        ...
        if path not in self.memo:
            ...
            self.memo.update({path: self._segment(*data) for path, data in
                              zip(paths, get_ninapro_db1_semg_binary_data(paths, self.preprocess))})
        semg, binary, local_gesture = self.memo[path][
            (self._as_local(combo.gesture), combo.trial)]
        nt.assert_equal(binary.shape[1], Dataset.num_binary)
        n = len(semg)
        gesture = np.repeat(combo.gesture, n)
        gesture[local_gesture == 0] = -1
        subject = np.repeat(combo.subject, n)
        return Trial(data=(semg, binary), gesture=gesture, subject=subject)

    def _segment(self, semg, binary, gesture):
        if self.norest:
        ...
        for begin, end in zip(breaks[:-1], breaks[1:]):
            ...
            if g > 0:
                ...
                data[(g, trials[g])] = (semg[begin:end], binary[begin:end], gesture[begin:end])
        return data
