...
class Dataset(BaseDataset):
    ...
    def get_dataiter(self, combos, **kargs):
        ...
        nt.assert_equal(len(semg), len(glove))
        nt.assert_equal(glove.ndim, 2)
...
class GetTrial(BaseGetTrial):
    ...
    def __call__(self, combo):
        ...
        nt.assert_equal(glove.shape[1], Dataset.num_glove)
        n = len(semg)
    ...
    def _segment(self, semg, glove, gesture):
        ...
        glove = np.hstack([glove, np.diff(np.vstack([glove[:1], glove]), axis=0)])
