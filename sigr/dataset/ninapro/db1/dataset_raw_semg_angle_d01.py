...
from .get_data import get_ninapro_db1_semg_angle_data
...
class Dataset(BaseDataset):
    ...
    def get_dataiter(self, combos, **kargs):
        ...
        assert semg and angle, 'Empty data'
        ...
        angle = np.vstack(angle)
        nt.assert_equal(len(semg), len(angle))
        nt.assert_equal(angle.ndim, 2)
    ...
    def __call__(self, combo):
        ...
        if path not in self.memo:
            ...
            self.memo.update({path: self._segment(*data) for path, data in
                              zip(paths, get_ninapro_db1_semg_angle_data(paths, self.preprocess))})
        semg, angle, local_gesture = self.memo[path][
            (self._as_local(combo.gesture), combo.trial)]
        nt.assert_equal(angle.shape[1], Dataset.num_angle)
        n = len(semg)
        gesture = np.repeat(combo.gesture, n)
        gesture[local_gesture == 0] = -1
        subject = np.repeat(combo.subject, n)
        return Trial(data=(semg, angle), gesture=gesture, subject=subject)

    def _segment(self, semg, angle, gesture):
        if self.norest:
        ...
        angle = np.hstack([angle, np.diff(np.vstack([angle[:1], angle]), axis=0)])
        ...
        for begin, end in zip(breaks[:-1], breaks[1:]):
            ...
            if g > 0:
                ...
                data[(g, trials[g])] = (semg[begin:end], angle[begin:end], gesture[begin:end])
        return data
