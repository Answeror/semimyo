...
class Symbol(FastLocallyConnectedMixin, BaseSymbol):
    ...
    def get_gesture_branch(self, pose_net, data, text, gesture_loss_weight):
        net = data
        net = self.get_branch(net, text, self.num_gesture, 'gesture')
        ...
        bias = mx.symbol.Variable(
            'pose_conditioned_gesture_bias',
            attr={'__shape__': str((self.num_pose, 1, self.num_gesture))}
        )
        net = mx.symbol.broadcast_plus(net, bias)
    ...
    def get_pose_branch(self, data, text, pose_loss_weight):
        net = data
        net = self.get_branch(net, text, self.num_pose, 'pose')
        net = self.get_softmax(data=net,
                               name='pose_softmax',
                               label=mx.symbol.Variable('pose'),
                               grad_scale=pose_loss_weight)
        net = self.get_average(net)
        return net
    ...
    def get_gesture_output(self, gesture_net, latent_net):
        ...
        gesture_net = self.get_average(gesture_net)
        ...
        assert_equal(self.infer_shape(gesture_net)[0], self.batch_size)
        return gesture_net

    def sample_data(self, data):
        shape = self.infer_shape(data)
        data = mx.symbol.Reshape(data, shape=(1,) + shape)
        data = mx.symbol.broadcast_axis(data, axis=0, size=self.num_mc_sample)
        data = mx.symbol.Reshape(data, shape=(self.num_mc_sample * self.batch_size,) + shape[1:])
        return data

    def get_average(self, data):
        net = data
        shape = self.infer_shape(net)
        net = mx.symbol.Reshape(net, shape=(self.num_mc_sample, self.batch_size) + shape[1:])
        net = mx.symbol.sum(net, axis=0) / self.num_mc_sample
        return net
