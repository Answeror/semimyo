...
class Symbol(symbol_base.Symbol):
    ...
    def __init__(
        ...
        **kargs
        ...
        gesture_branch = self.get_gesture_branch(
            ...
            num_gesture,
            gesture_loss_weight,
            pose_branch.get_internals()['pose_last_fc_output']
        )
    ...
    def get_gesture_branch(self, data, net, num_gesture, gesture_loss_weight, pose_net):
        net = self.get_branch(data, net, num_gesture, 'gesture')
        net = mx.symbol.Concat(net, mx.symbol.BlockGrad(pose_net), axis=1)
        if self.for_training:
