...
class Symbol(BaseSymbol):
    ...
    def __init__(self, **kargs):
        ...
        if self.for_training:
            loss = [gesture_branch,
                    self.get_dist_branch(self.get_shortnet('id:dist_branch', shared_net))]
    ...
    def infer_shape(self, data):
        ...
        shape = dict(
            ...
            dist_mask=(self.batch_size, self.num_semg_row * self.num_semg_col, 2),
    ...
    def get_dist_branch(self, data):
        net = data
        dist = mx.sym.Variable('dist')
        dist_mask = mx.sym.Variable('dist_mask')
        net = mx.sym.Reshape(net, shape=(0, 0, -1))
        net = mx.sym.batch_dot(net, dist_mask)
        net = self.get_shortnet(self.dist_net, net, prefix='dist')
        net = mx.sym.sum(mx.sym.square(net - dist), axis=1)
        net = mx.sym.MakeLoss(net, grad_scale=self.dist_loss_weight)
        return net
