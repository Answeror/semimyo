...
class Symbol(BaseSymbol):
    ...
    def __init__(
        ...
        **kargs
        ...
        self.num_stochastic_sample = num_stochastic_sample or 5
        ...
        if self.for_training:
            ...
            loss.append(self.get_smooth_loss(
                mx.symbol.SoftmaxActivation(self.get_shortnet(
                    'id:gesture_smooth_loss_branch',
                    mx.symbol.Concat(
                        gesture_branch.get_internals()['gesture_last_fc_output'],
                        *[self.get_shortnet(self.stochastic_net,
                                            shared_net.get_internals()[self.stochastic_input],
                                            prefix='mirror%d' % i)
                          for i in range(self.num_stochastic_sample - 1)],
                        dim=0
                    )
                )),
                'gesture'
    ...
    def get_smooth_loss(self, data, tag):
        ...
        net = mx.symbol.Reshape(
            net,
            shape=(self.num_stochastic_sample, self.batch_size, self.window, -1)
        )
        net = mx.symbol.sum(net, axis=0) / self.num_stochastic_sample
        net = mx.symbol.SliceChannel(
            net,
            axis=1,
            num_outputs=self.window
