...
class Symbol(BaseSymbol):
    ...
    def __init__(
        ...
        **kargs
        ...
        has_angle_branch = angle_loss_weight > 0
        ...
        if self.for_training and has_angle_branch:
            angle_branch = self.get_angle_branch(
                self.get_shortnet('id:angle_branch', shared_net),
                angle_net,
                angle_loss_weight
            )
            if has_gesture_branch:
                self.net = mx.symbol.Group([gesture_branch, angle_branch])
            else:
                self.net = angle_branch
        else:
    ...
    def infer_shape(self, data):
        ...
        shape = dict(
            ...
            angle=(self.batch_size, self.num_angle)
        )
    ...
    def get_angle_branch(self, data, net, angle_loss_weight):
        net = self.get_shortnet(net, data, prefix='angle')
        angle = mx.sym.Variable('angle')
        angle = self.get_shortnet(
            getattr(self, 'angle_label_net', None),
            angle,
            prefix='angle_label'
        )
        net = mx.sym.sum(mx.sym.square(net - angle), axis=1) * (1 / self.num_angle)
        net = mx.sym.MakeLoss(net, grad_scale=angle_loss_weight)
        return net
